...
$url = new moodle_url('/mod/survey/show_entries.php', array('id'=>$cm->id, 'do_show'=>$do_show));
...
if ($do_show == 'showoneentry') {
    ...
    $params = array('survey'=>$survey->id,
                    'userid'=>$userid,
                    'anonymous_response'=>SURVEY_ANONYMOUS_NO);

    $surveycompleted = $DB->get_record('survey_completed', $params); //arb
}
...
if ($do_show == 'showentries') {
    ...
    if (has_capability('mod/survey:viewreports', $context)) {
        ...
        $baseurl = new moodle_url('/mod/survey/show_entries.php');
        ...
        $table = new flexible_table('survey-showentry-list-'.$course->id);
        ...
        $matchcount = survey_count_complete_users($cm, $usedgroupid);
        ...
        $students = survey_get_complete_users($cm, $usedgroupid, $where, $params, $sort, $startpage, $pagecount);
        $str_analyse = get_string('analysis', 'survey');
        $str_complete = get_string('completed_surveys', 'survey');
        $str_course = get_string('course');

        $completed_fb_count = survey_get_completeds_group_count($survey, $mygroupid);
        if ($survey->course == SITEID) {
            $analysisurl = new moodle_url('/mod/survey/analysis_course.php', array('id'=>$id, 'courseid'=>$courseid));
            echo $OUTPUT->box_start('mdl-align');
            echo '<a href="'.$analysisurl->out().'">';
            echo $str_course.' '.$str_analyse.' ('.$str_complete.': '.intval($completed_fb_count).')';
            echo '</a>';
            echo $OUTPUT->help_icon('viewcompleted', 'survey');
            echo $OUTPUT->box_end();
        } else {
            $analysisurl = new moodle_url('/mod/survey/analysis.php', array('id'=>$id, 'courseid'=>$courseid));
            echo $OUTPUT->box_start('mdl-align');
            echo '<a href="'.$analysisurl->out().'">';
    ...
    if (has_capability('mod/survey:viewreports', $context)) {
        ...
        if (!$students) {
            ...
            echo print_string('non_anonymous_entries', 'survey');
            ...
            foreach ($students as $student) {
                ...
                $completed_count = $DB->count_records('survey_completed', $params);
                if ($completed_count > 0) {
                    ...
                    $surveycompleted = $DB->get_record('survey_completed', $params);
                    $showentryurl_params = array('userid'=>$student->id, 'do_show'=>'showoneentry');
                    $showentryurl = new moodle_url($url, $showentryurl_params);
                    $showentrylink = '<a href="'.$showentryurl->out().'">'.userdate($surveycompleted->timemodified).'</a>';
                    $data[] = $showentrylink;

                    //link to delete the entry
                    if (has_capability('mod/survey:deletesubmissions', $context)) {
                        $delete_url_params = array('id' => $cm->id,
                                            'completedid' => $surveycompleted->id,
                                            'do_show' => 'showoneentry');

                        $deleteentryurl = new moodle_url($CFG->wwwroot.'/mod/survey/delete_completed.php', $delete_url_params);
                        $deleteentrylink = '<a href="'.$deleteentryurl->out().'">'.
                                                get_string('delete_entry', 'survey').
                                            '</a>';
        ...
        <table width="100%">
            <tr>
                <td align="left" colspan="2">
                    ...
                    $survey_completeds_count = $DB->count_records('survey_completed', $params);
                    print_string('anonymous_entries', 'survey');
                    echo '&nbsp;('.$survey_completeds_count.')';
                    ?>
                </td>
                <td align="right">
                    <?php
                        ...
                        echo $OUTPUT->single_button($aurl, get_string('show_entries', 'survey'));
                    ?>
                </td>
            </tr>
        </table>
...
if ($do_show == 'showoneentry') {
    ...
    if (is_array($surveyitems)) {
        ...
        if ($surveycompleted) {
            echo $OUTPUT->heading(userdate($surveycompleted->timemodified).' ('.fullname($usr).')', 3);
        } else {
            echo $OUTPUT->heading(get_string('not_completed_yet', 'survey'), 3);
        }
        ...
        foreach ($surveyitems as $surveyitem) {
            ...
            if ($surveyitem->typ != 'pagebreak') {
                ...
                if (isset($value->value)) {
                    survey_print_item_show_value($surveyitem, $value->value);
                } else {
                    survey_print_item_show_value($surveyitem, false);
                }
                echo $OUTPUT->box_end();
            }
            echo $OUTPUT->box_end();
