...
$url = new moodle_url('/mod/survey/delete_template.php', array('id'=>$id));
...
if ($canceldelete == 1) {
    $editurl = new moodle_url('/mod/survey/edit.php', array('id'=>$id, 'do_show'=>'templates'));
...
require_capability('mod/survey:deletetemplate', $context);

$mform = new mod_survey_delete_template_form();
...
if (isset($formdata->confirmdelete) AND $formdata->confirmdelete == 1) {
    if (!$template = $DB->get_record("survey_template", array("id"=>$deletetempl))) {
        print_error('error');
    }

    if ($template->ispublic) {
        $systemcontext = get_system_context();
        require_capability('mod/survey:createpublictemplate', $systemcontext);
        require_capability('mod/survey:deletetemplate', $systemcontext);
    }

    survey_delete_template($template);
    redirect($deleteurl->out(false));
}
...
$strdeletesurvey = get_string('delete_template', 'survey');
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading($strdeletesurvey);
if ($shoulddelete == 1) {

    echo $OUTPUT->box_start('generalbox errorboxcontent boxaligncenter boxwidthnormal');
    echo $OUTPUT->heading(get_string('confirmdeletetemplate', 'survey'));
    $mform->display();
    echo $OUTPUT->box_end();
} else {
    //first we get the own templates
    $templates = survey_get_template_list($course, 'own');
    if (!is_array($templates)) {
        echo $OUTPUT->box(get_string('no_templates_available_yet', 'survey'),
                         'generalbox boxaligncenter');
    } else {
        echo $OUTPUT->heading(get_string('course'), 3);
        echo $OUTPUT->box_start('generalbox boxaligncenter boxwidthnormal');
        $tablecolumns = array('template', 'action');
        $tableheaders = array(get_string('template', 'survey'), '');
        $tablecourse = new flexible_table('survey_template_course_table');
        ...
        foreach ($templates as $template) {
            ...
            $data[] = $OUTPUT->single_button($url, $strdeletesurvey, 'post');
    ...
    $systemcontext = get_system_context();
    if (has_capability('mod/survey:createpublictemplate', $systemcontext) AND
        has_capability('mod/survey:deletetemplate', $systemcontext)) {
        $templates = survey_get_template_list($course, 'public');
        if (!is_array($templates)) {
            echo $OUTPUT->box(get_string('no_templates_available_yet', 'survey'),
                              'generalbox boxaligncenter');
        } else {
            echo $OUTPUT->heading(get_string('public', 'survey'), 3);
            echo $OUTPUT->box_start('generalbox boxaligncenter boxwidthnormal');
            $tablecolumns = array('template', 'action');
            $tableheaders = array(get_string('template', 'survey'), '');
            $tablepublic = new flexible_table('survey_template_public_table');
