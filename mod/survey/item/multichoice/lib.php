...
define('SURVEY_MULTICHOICE_TYPE_SEP', '>>>>>');
define('SURVEY_MULTICHOICE_LINE_SEP', '|');
define('SURVEY_MULTICHOICE_ADJUST_SEP', '<<<<<');
define('SURVEY_MULTICHOICE_IGNOREEMPTY', 'i');
define('SURVEY_MULTICHOICE_HIDENOSELECT', 'h');

class survey_item_multichoice extends survey_item_base {
    ...
    public function build_editform($item, $survey, $cm) {
        ...
        $this->item_form = new survey_multichoice_form('edit_item.php', $customdata);
    }
    ...
    public function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $answers = null;
        $answers = explode (SURVEY_MULTICHOICE_LINE_SEP, $info->presentation);
        if (!is_array($answers)) {
            return null;
        }
        ...
        if ($info->subtype == 'c') {
            ...
            for ($i = 1; $i <= $sizeofanswers; $i++) {
                ...
                foreach ($values as $value) {
                    ...
                    $vallist = explode(SURVEY_MULTICHOICE_LINE_SEP, $value->value);
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode (SURVEY_MULTICHOICE_LINE_SEP, $info->presentation);

        if ($info->subtype == 'c') {
            $vallist = array_values(explode (SURVEY_MULTICHOICE_LINE_SEP, $value->value));
        ...
        if ($analysed_item) {
            ...
            foreach ($analysed_vals as $val) {
                ...
                echo '<td align="left" valign="top">
                      ...
                      <td align="left" style="width:'.SURVEY_MAX_PIX_LENGTH.';">
    ...
    public function print_item_complete($item, $value = null, $highlightrequire = false) {
        ...
        // if ($info->subtype == 'c') {
            if (is_array($value)) {
                ...
                $values = explode(SURVEY_MULTICHOICE_LINE_SEP, $value);
            }
            $highlight = '';
    ...
    public function print_item_show_value($item, $value = null) {
        ...
        $requiredmark = '';
        if ($item->required == 1) {
            $requiredmark = '<span class="survey_required_mark">*</span>';
        }
        ...
        if ($info->subtype == 'c') {
            ...
            foreach ($presentation as $pres) {
                foreach ($values as $val) {
                    if ($val == $index) {
                        echo '<div class="survey_item_multianswer">';
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(SURVEY_MULTICHOICE_LINE_SEP, $dbvalue);
        }
    ...
    public function get_presentation($data) {
        $present = str_replace("\n", SURVEY_MULTICHOICE_LINE_SEP, trim($data->itemvalues));
        ...
        if (isset($data->horizontal) AND $data->horizontal == 1 AND $subtype != 'd') {
            $present .= SURVEY_MULTICHOICE_ADJUST_SEP.'1';
        }
        return $subtype.SURVEY_MULTICHOICE_TYPE_SEP.$present;
    }
    ...
    private function get_info($item) {
        ...
        $parts = explode(SURVEY_MULTICHOICE_TYPE_SEP, $item->presentation);
        ...
        if ($info->subtype != 'd') {
            $parts = explode(SURVEY_MULTICHOICE_ADJUST_SEP, $info->presentation);
    ...
    private function item_array_to_string($value) {
        ...
        for ($i = 1; $i < $sizeofarrvals; $i++) {
            $retval .= SURVEY_MULTICHOICE_LINE_SEP.$arrvals[$i];
        }
        return $retval;
    }
    ...
    private function print_item_check($presentation, $item, $value, $info, $align) {
        ...
        ?>
            <li class="survey_item_check_<?php echo $hv.'_'.$align;?>">
                <span class="survey_item_check_<?php echo $hv.'_'.$align;?>">
                    <?php
    ...
    public function set_ignoreempty($item, $ignoreempty=true) {
        $item->options = str_replace(SURVEY_MULTICHOICE_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= SURVEY_MULTICHOICE_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, SURVEY_MULTICHOICE_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect=true) {
        $item->options = str_replace(SURVEY_MULTICHOICE_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= SURVEY_MULTICHOICE_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options, SURVEY_MULTICHOICE_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
