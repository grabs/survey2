...
$url = new moodle_url('/mod/survey/import.php', array('id'=>$id));
if ($choosefile !== false) {
...
if ($choosefile) {
    ...
    if (!$xmldata = survey_load_xml_data($xmlcontent)) {
        print_error('cannotloadxml', 'survey', 'edit.php?id='.$id);
    }

    $importerror = survey_import_loaded_data($xmldata, $survey->id);
    if ($importerror->stat == true) {
        $url = 'edit.php?id='.$id.'&do_show=templates';
        redirect($url, get_string('import_successfully', 'survey'), 3);
        exit;
    }
}
...
///////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////
echo $OUTPUT->heading(get_string('import_questions', 'survey'));
...
function survey_load_xml_data($xmlcontent) {
    global $CFG;
    require_once($CFG->dirroot.'/lib/xmlize.php');

    if (!$xmlcontent = survey_check_xml_utf8($xmlcontent)) {
        return false;
    }

    $data = xmlize($xmlcontent, 1, 'UTF-8');

    if (intval($data['SURVEY']['@']['VERSION']) != 200701) {
        return false;
    }
    $data = $data['SURVEY']['#']['ITEMS'][0]['#']['ITEM'];
    return $data;
}

function survey_import_loaded_data(&$data, $surveyid) {
    global $CFG, $DB;
    ...
    if (!is_array($data)) {
        $error->msg[] = get_string('data_is_not_an_array', 'survey');
        $error->stat = false;
        return $error;
    }

    if ($deleteolditems) {
        survey_delete_all_items($surveyid);
        $position = 0;
    } else {
        //items will be add to the end of the existing items
        $position = $DB->count_records('survey_item', array('survey'=>$surveyid));
    }
    ...
    foreach ($data as $item) {
        ...
        $newid = $DB->insert_record('survey_item', $newitem);
...
function survey_check_xml_utf8($text) {
    //find the encoding
