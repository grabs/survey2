...
$url = new moodle_url('/mod/survey/export.php', array('id'=>$id));
if ($action !== false) {
...
if ($action == 'exportfile') {
    if (!$exportdata = survey_get_xml_data($survey->id)) {
        print_error('nodata');
    }
    @survey_send_xml_data($exportdata, 'survey_'.$survey->id.'.xml');
    exit;
}
...
function survey_get_xml_data($surveyid) {
    global $DB;

    $space = '     ';
    //get all items of the survey
    if (!$items = $DB->get_records('survey_item', array('survey'=>$surveyid), 'position')) {
        return false;
    }

    //writing the header of the xml file including the charset of the currrent used language
    $data = '<?xml version="1.0" encoding="UTF-8" ?>'."\n";
    $data .= '<SURVEY VERSION="200701" COMMENT="XML-Importfile for mod/survey">'."\n";
    $data .= $space.'<ITEMS>'."\n";
...
function survey_send_xml_data($data, $filename) {
