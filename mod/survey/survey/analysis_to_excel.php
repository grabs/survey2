...
/**
 * prints an analysed excel-spreadsheet of the survey
 *
...
survey_load_survey_items();

$id = required_param('id', PARAM_INT);  //the POST dominated the GET
$coursefilter = optional_param('coursefilter', '0', PARAM_INT);

$url = new moodle_url('/mod/survey/analysis_to_excel.php', array('id'=>$id));
if ($coursefilter !== '0') {
...
$fstring->bold = get_string('bold', 'survey');
$fstring->page = get_string('page', 'survey');
$fstring->of = get_string('of', 'survey');
$fstring->modulenameplural = get_string('modulenameplural', 'survey');
$fstring->questions = get_string('questions', 'survey');
$fstring->itemlabel = get_string('item_label', 'survey');
$fstring->question = get_string('question', 'survey');
$fstring->responses = get_string('responses', 'survey');
$fstring->idnumber = get_string('idnumber');
$fstring->username = get_string('username');
$fstring->fullname = get_string('fullnameuser');
$fstring->courseid = get_string('courseid', 'survey');
$fstring->course = get_string('course');
$fstring->anonymous_user = get_string('anonymous_user', 'survey');
ob_end_clean();
...
if (!$items = $DB->get_records('survey_item', $params, 'position')) {
    print_error('no_items_available_yet',
                'survey',
                $CFG->wwwroot.'/mod/survey/view.php?id='.$id);
    exit;
}
...
$sheetname = clean_param($survey->name, PARAM_ALPHANUM);
error_reporting(0);
...
$completeds = survey_get_completeds_group($survey, $mygroupid, $coursefilter);
//important: for each completed you have to print each item, even if it is not filled out!!!
//therefor for each completed we have to iterate over all items of the survey
//this is done by survey_excelprint_detailed_items

$row_offset2 = 0;
//first we print the table-header
$row_offset2 = survey_excelprint_detailed_head($worksheet2, $xls_formats, $items, $row_offset2);


if (is_array($completeds)) {
    foreach ($completeds as $completed) {
        $row_offset2 = survey_excelprint_detailed_items($worksheet2,
                                                         $xls_formats,
                                                         $completed,
                                                         $items,
                                                         $row_offset2);
...
function survey_excelprint_detailed_head(&$worksheet, $xls_formats, $items, $row_offset) {
    global $fstring, $survey;
...
function survey_excelprint_detailed_items(&$worksheet, $xls_formats,
    ...
    $survey = $DB->get_record('survey', array('id'=>$completed->survey));
    //get the username
    ...
    if ($user = $DB->get_record('user', array('id'=>$completed->userid))) {
        if ($completed->anonymous_response == SURVEY_ANONYMOUS_NO) {
