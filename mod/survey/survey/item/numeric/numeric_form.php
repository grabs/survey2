...
class survey_numeric_form extends survey_item_form {
    ...
    public function definition() {
        ...
        $mform->addElement('text',
                            'rangefrom',
                            get_string('numeric_range_from', 'survey'),
                            array('size'=>10, 'maxlength'=>10));

        $mform->addElement('text',
                            'rangeto',
                            get_string('numeric_range_to', 'survey'),
                            array('size'=>10, 'maxlength'=>10));
    ...
    public function get_data() {
        ...
        $itemobj = new survey_item_numeric();

        $num1 = str_replace($itemobj->sep_dec, SURVEY_DECIMAL, $item->rangefrom);
        if (is_numeric($num1)) {
            $num1 = floatval($num1);
        } else {
            $num1 = '-';
        ...
        $num2 = str_replace($itemobj->sep_dec, SURVEY_DECIMAL, $item->rangeto);
        if (is_numeric($num2)) {
            $num2 = floatval($num2);
        } else {
            $num2 = '-';
