...
require_once($CFG->dirroot.'/mod/survey/item/survey_item_class.php');

define('SURVEY_RADIORATED_ADJUST_SEP', '<<<<<');

define('SURVEY_MULTICHOICERATED_MAXCOUNT', 10); //count of possible items
define('SURVEY_MULTICHOICERATED_VALUE_SEP', '####');
define('SURVEY_MULTICHOICERATED_VALUE_SEP2', '/');
define('SURVEY_MULTICHOICERATED_TYPE_SEP', '>>>>>');
define('SURVEY_MULTICHOICERATED_LINE_SEP', '|');
define('SURVEY_MULTICHOICERATED_ADJUST_SEP', '<<<<<');
define('SURVEY_MULTICHOICERATED_IGNOREEMPTY', 'i');
define('SURVEY_MULTICHOICERATED_HIDENOSELECT', 'h');

class survey_item_multichoicerated extends survey_item_base {
    ...
    public function build_editform($item, $survey, $cm) {
        global $DB, $CFG;
        require_once('multichoicerated_form.php');

        //get the lastposition number of the survey_items
        $position = $item->position;
        $lastposition = $DB->count_records('survey_item', array('survey'=>$survey->id));
        if ($position == -1) {
        ...
        $surveyitems = survey_get_depend_candidates_for_item($survey, $item);
        ...
        $this->item_form = new survey_multichoicerated_form('edit_item.php', $customdata);
    }
    ...
    public function save_item() {
        ...
        if (!$item->id) {
            ...
            $DB->update_record('survey_item', $item);
        }

        return $DB->get_record('survey_item', array('id'=>$item->id));
    }
    ...
    public function get_analysed($item, $groupid = false, $courseid = false) {
        ...
        $info = $this->get_info($item);
        $lines = null;
        $lines = explode (SURVEY_MULTICHOICERATED_LINE_SEP, $info->presentation);
        if (!is_array($lines)) {
            return null;
        }

        //die Werte holen
        $values = survey_get_group_values($item, $groupid, $courseid, $this->ignoreempty($item));
        if (!$values) {
            return null;
        }
        ...
        for ($i = 1; $i <= $sizeoflines; $i++) {
            $item_values = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $lines[$i-1]);
            $ans = null;
    ...
    public function get_printval($item, $value) {
        ...
        $presentation = explode (SURVEY_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $index = 1;
        foreach ($presentation as $pres) {
            if ($value->value == $index) {
                $item_label = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $pres);
    ...
    public function print_analysed($item, $itemnr = '', $groupid = false, $courseid = false) {
        ...
        if ($analysed_item) {
            ...
            foreach ($analysed_vals as $val) {
                ...
                $pixnr++;
                $pixwidth = intval($val->quotient * SURVEY_MAX_PIX_LENGTH);
                ...
                echo '<td align="left" style="width: '.SURVEY_MAX_PIX_LENGTH.'">';
            ...
            echo get_string('average', 'survey').': '.$avg.'</b>';
            echo '</td></tr>';
        }
    }
    ...
    /**
     * print the item at the edit-page of survey
     *
     * @global object
     * @param object $item
     * @return void
    ...
    public function print_item_preview($item) {
        ...
        $info = $this->get_info($item);
        $str_required_mark = '<span class="survey_required_mark">*</span>';

        $lines = explode (SURVEY_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $requiredmark =  ($item->required == 1) ? $str_required_mark : '';
        //print the question and label
        echo '<div class="survey_item_label_'.$align.'">';
        echo '('.$item->label.') ';
        echo format_text($item->name.$requiredmark, true, false, false);
        if ($item->dependitem) {
            if ($dependitem = $DB->get_record('survey_item', array('id'=>$item->dependitem))) {
                echo ' <span class="survey_depend">';
        ...
        echo '<div class="survey_item_presentation_'.$align.'">';
        switch($info->subtype) {
            case 'r':
    ...
    /**
     * print the item at the complete-page of survey
     *
     * @global object
     * @param object $item
     * @param string $value
    ...
    public function print_item_complete($item, $value = '', $highlightrequire = false) {
        ...
        //print the question and label
        echo '<div class="survey_item_label_'.$align.$highlight.'">';
            echo format_text($item->name.$requiredmark, true, false, false);
        echo '</div>';

        //print the presentation
        echo '<div class="survey_item_presentation_'.$align.$highlight.'">';
        switch($info->subtype) {
            case 'r':
    ...
    public function print_item_show_value($item, $value = '') {
        ...
        $lines = explode (SURVEY_MULTICHOICERATED_LINE_SEP, $info->presentation);
        $requiredmark = ($item->required == 1)?'<span class="survey_required_mark">*</span>':'';
        ...
        foreach ($lines as $line) {
            if ($value == $index) {
                $item_value = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $line);
    ...
    public function compare_value($item, $dbvalue, $dependvalue) {
        ...
        if (is_array($dbvalue)) {
            ...
            $dbvalues = explode(SURVEY_MULTICHOICERATED_LINE_SEP, $dbvalue);
        }
        ...
        $index = 1;
        foreach ($presentation as $pres) {
            $presvalues = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $pres);
    ...
    public function get_presentation($data) {
        ...
        if (isset($data->horizontal) AND $data->horizontal == 1 AND $subtype != 'd') {
            $present .= SURVEY_MULTICHOICERATED_ADJUST_SEP.'1';
        }
        return $subtype.SURVEY_MULTICHOICERATED_TYPE_SEP.$present;
    }
    ...
    private function get_info($item) {
        ...
        $parts = explode(SURVEY_MULTICHOICERATED_TYPE_SEP, $item->presentation);
        ...
        if ($info->subtype != 'd') {
            $parts = explode(SURVEY_MULTICHOICERATED_ADJUST_SEP, $info->presentation);
    ...
    private function print_item_radio($item, $value, $info, $align, $showrating, $lines) {
        ...
        if (!$this->hidenoselect($item)) {
            ?>
                <li class="survey_item_radio_<?php echo $hv.'_'.$align;?>">
                    <span class="survey_item_radio_<?php echo $hv.'_'.$align;?>">
                        <?php
                    ...
                    <span class="survey_item_radiolabel_<?php echo $hv.'_'.$align;?>">
                        <label for="<?php echo $item->typ . '_' . $item->id.'_xxx';?>">
                        <?php print_string('not_selected', 'survey');?>&nbsp;
                        </label>
                    </span>
                </li>
            <?php
        ...
        foreach ($lines as $line) {
            ...
            $radio_value = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $line);
    ...
    private function print_item_dropdown($item, $value, $info, $align, $showrating, $lines) {
        ...
        echo '<ul>';
        ?>
        <li class="survey_item_select_<?php echo $hv.'_'.$align;?>">
            <select name="<?php echo $item->typ.'_'.$item->id;?>">
                ...
                foreach ($lines as $line) {
                    ...
                    $dropdown_value = explode(SURVEY_MULTICHOICERATED_VALUE_SEP, $line);
                    if ($showrating) {
    ...
    public function prepare_presentation_values_print($valuestring, $valuesep1, $valuesep2) {
        return $this->prepare_presentation_values(SURVEY_MULTICHOICERATED_LINE_SEP,
                                                  "\n",
                                                  $valuestring,
                                                  $valuesep1,
                                                  $valuesep2);
    ...
    public function prepare_presentation_values_save($valuestring, $valuesep1, $valuesep2) {
        return $this->prepare_presentation_values("\n",
                        SURVEY_MULTICHOICERATED_LINE_SEP,
                        $valuestring,
                        $valuesep1,
                        $valuesep2);
    }
    ...
    public function set_ignoreempty($item, $ignoreempty=true) {
        $item->options = str_replace(SURVEY_MULTICHOICERATED_IGNOREEMPTY, '', $item->options);
        if ($ignoreempty) {
            $item->options .= SURVEY_MULTICHOICERATED_IGNOREEMPTY;
        }
    }

    public function ignoreempty($item) {
        if (strstr($item->options, SURVEY_MULTICHOICERATED_IGNOREEMPTY)) {
            return true;
        }
        return false;
    }
    ...
    public function set_hidenoselect($item, $hidenoselect=true) {
        $item->options = str_replace(SURVEY_MULTICHOICERATED_HIDENOSELECT, '', $item->options);
        if ($hidenoselect) {
            $item->options .= SURVEY_MULTICHOICERATED_HIDENOSELECT;
        }
    }

    public function hidenoselect($item) {
        if (strstr($item->options, SURVEY_MULTICHOICERATED_HIDENOSELECT)) {
            return true;
        }
        return false;
    }
