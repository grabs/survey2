...
define('SURVEY_ITEM_NAME_TEXTBOX_SIZE', 80);
define('SURVEY_ITEM_LABEL_TEXTBOX_SIZE', 20);
abstract class survey_item_form extends moodleform {
    public function definition() {
        ...
        if ($common['items']) {
            $mform->addElement('select',
                                'dependitem',
                                get_string('dependitem', 'survey').'&nbsp;',
                                $common['items']
                                );
            $mform->addHelpButton('dependitem', 'depending', 'survey');
            $mform->addElement('text',
                                'dependvalue',
                                get_string('dependvalue', 'survey'),
        ...
        $mform->addElement('hidden', 'survey', $common['survey']);
        $mform->setType('survey', PARAM_INT);
        ...
        if (!empty($item->id)) {
            $buttonarray[] = &$mform->createElement('submit',
                                                    'update_item',
                                                    get_string('update_item', 'survey'));

            $buttonarray[] = &$mform->createElement('submit',
                                                    'clone_item',
                                                    get_string('save_as_new_item', 'survey'));
        } else {
            $mform->addElement('hidden', 'clone_item', 0);
            $mform->setType('clone_item', PARAM_INT);
            $buttonarray[] = &$mform->createElement('submit',
                                                    'save_item',
                                                    get_string('save_item', 'survey'));
        }
