...
class survey_item_captcha extends survey_item_base {
    ...
    public function build_editform($item, $survey, $cm) {
        ...
        $editurl = new moodle_url('/mod/survey/edit.php', array('id'=>$cm->id));

        //ther are no settings for recaptcha
        if (isset($item->id) AND $item->id > 0) {
            notice(get_string('there_are_no_settings_for_recaptcha', 'survey'), $editurl->out());
            exit;
        }

        //only one recaptcha can be in a survey
        $params = array('survey' => $survey->id, 'typ' => $this->type);
        if ($DB->record_exists('survey_item', $params)) {
            notice(get_string('only_one_captcha_allowed', 'survey'), $editurl->out());
            exit;
        }
        ...
        $this->item->survey = $survey->id;
        $this->item->template = 0;
        $this->item->name = get_string('captcha', 'survey');
        $this->item->label = get_string('captcha', 'survey');
    ...
    public function save_item() {
        ...
        if (empty($this->item->id)) {
            $this->item->id = $DB->insert_record('survey_item', $this->item);
        } else {
            $DB->update_record('survey_item', $this->item);
        }

        return $DB->get_record('survey_item', array('id'=>$this->item->id));
    }
    ...
    public function print_item_preview($item) {
        ...
        if ($surveyid > 0) {
            $survey = $DB->get_record('survey', array('id'=>$surveyid));
            $cm = get_coursemodule_from_instance("survey", $survey->id, $survey->course);
            if ($cm) {
                $cmid = $cm->id;
            }
        }
    ...
    public function print_item_complete($item, $value = '', $highlightrequire = false) {
        ...
        if (isset($SESSION->survey->captchacheck) AND
                $SESSION->survey->captchacheck == $USER->sesskey AND
                $value == $USER->sesskey) {
    ...
    public function print_item_show_value($item, $value = '') {
        ...
        if ($surveyid > 0) {
            $survey = $DB->get_record('survey', array('id'=>$surveyid));
            if ($cm = get_coursemodule_from_instance("survey", $survey->id, $survey->course)) {
                $cmid = $cm->id;
            }
        }
    ...
    public function check_value($value, $item) {
        ...
        if ($response->is_valid) {
            $SESSION->survey->captchacheck = $USER->sesskey;
            return true;
        }
        unset($SESSION->survey->captchacheck);
