...
class survey_item_label extends survey_item_base {
    ...
    public function build_editform($item, $survey, $cm) {
        ...
        $this->item_form = new survey_label_form('edit_item.php', $customdata);
    }
    ...
    public function print_item($item) {
        ...
        if (!$item->survey AND $item->template) {
            $template = $DB->get_record('survey_template', array('id'=>$item->template));
            ...
            $cm = get_coursemodule_from_instance('survey', $item->survey);
    ...
    //when provided $data submitted from survey_show_edit
