...
/**
 * print the form to map courses for global surveys
 *
...
$url = new moodle_url('/mod/survey/mapcourse.php', array('id'=>$id));
if ($searchcourse !== '') {
...
require_capability('mod/survey:mapcourse', $context);

if ($coursefilter) {
    $map->surveyid = $survey->id;
    $map->courseid = $coursefilter;
    // insert a map only if it does exists yet
    $sql = "SELECT id, surveyid
              FROM {survey_sitecourse_map}
             WHERE surveyid = ? AND courseid = ?";
    if (!$DB->get_records_sql($sql, array($map->surveyid, $map->courseid))) {
        $DB->insert_record('survey_sitecourse_map', $map);
    }
}
...
echo $OUTPUT->box(get_string('mapcourseinfo', 'survey'), 'generalbox boxaligncenter boxwidthwide');
...
if (($courses = $DB->get_records_sql_menu($sql, $params)) && !empty($searchcourse)) {
    ...
    echo '<input type="submit" value="'.get_string('mapcourse', 'survey').'"/>';
    echo $OUTPUT->help_icon('mapcourses', 'survey');
    ...
    echo $OUTPUT->help_icon('searchcourses', 'survey');
} else {
...
if ($coursemap = survey_get_courses_from_sitecourse_map($survey->id)) {
    $table = new flexible_table('coursemaps');
    $table->define_columns( array('course'));
    $table->define_headers( array(get_string('mappedcourses', 'survey')));

    $table->setup();

    $unmapurl = new moodle_url('/mod/survey/unmapcourse.php');
    ...
    echo '<h3>'.get_string('mapcoursenone', 'survey').'</h3>';
}
