...
Overview
--------
The Survey-Module is intended to create individual surveys in an easy way.

This module consists of two parts
a) the "activity" survey (required)
b) the "sideblock" survey (optional)
...
an central placed survey-activity. So you can create one survey on the main site of moodle and then publish
it in many courses.
...
------------
   ...
   copy the folder survey.zip/mod/survey --> moodle/mod/survey
   and the folder survey.zip/blocks/survey --> moodle/blocks/survey
   The langfiles normaly can be left into the folder mod/survey/lang.
   The only exception is the survey-block. The langfile is block_survey.php and
...
-----------------------
   1. create one or more new survey-activitys on the moodle main-site
   2. go into some course and enable the survey-block. This block now shows the surveys from the main-site.
   3. login as student and go into the course where the survey-block is enabled
   4. fill out the survey chosen from block
   5. login as admin and look at the survey you created above
...
03.04.2008
- anonymous survey was send the userid on email-notification
...
03.12.2007
- changing the table survey_template, field public -> ispublic, public is reserved in oracle
...
11.09.2007
- Update survey settings has help showing wrongly
...
- missing sesskey checks in survey
- uninitialised $filename when importing into survey
...
09.05.2007
- added two columns (random_response,anonymous_response) to survey_complete and survey_complete_tmp
- userids now will be logged even if the survey is anonymous so you now can filter by group
...
- a survey what is switch to the next page is saved temporary.
  the user can cancel the completion and later continue at the last filled page.
- now you can ex-/import surveys into/from a xml-file
...
02.06.2006 21:22
several bugfixes
improve the block "survey"

20.05.2006 01:00
adding the block "survey" to publish survey over all courses
Thanks to Jun Yamog!
...
21.04.2006 16:00
version 2006042101
prefixed all function-names like "survey_"
...
function survey_action_handler() argument list changed
...
If survey is not anonym now guest is it not allowed to fill it out

11.08.2005 22:00
added email-notification
anonymous surveys can be filled out by anonymous users

03.08.2005 01:20
item specific functions were moved into the items-librarys
now developers can create individual survey-items
