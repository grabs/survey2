...
require_once($CFG->dirroot . '/mod/survey/backup/moodle2/backup_survey_settingslib.php'); // Because it exists (optional)
...
class backup_survey_activity_task extends backup_activity_task {
    ...
    protected function define_my_steps() {
        ...
        $this->add_step(new backup_survey_activity_structure_step('survey structure', 'survey.xml'));
    }
    ...
    static public function encode_content_links($content) {
        ...
        // Link to survey analyis by moduleid
        $search="/(".$base."\/mod\/survey\/analysis.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@SURVEYANALYSISBYID*$2@$', $content);

        // Link to survey entries by moduleid
        $search="/(".$base."\/mod\/survey\/show_entries.php\?id\=)([0-9]+)/";
        $content= preg_replace($search, '$@SURVEYSHOWENTRIESBYID*$2@$', $content);
